...
def lambda_handler(event, context):
    # Create EC2 Client
    ec2_east_client = boto3.client("ec2", region_name="us-east-1")
    
    # Create RDS Client in East Region
    rds_east_client = boto3.client("rds", region_name="us-east-1")
    ...
    for inst in resources_info["Items"]:
        ...
        if inst.get("resource_type") == "EC2":
        ...
        elif inst.get("resource_type") == "RDS":
            # Launch DB Cluster
            ...
            try:
                ...
                vDBName = ""
                if RDS_Data.get("DBName") != None:
                    vDBName = RDS_Data["DBName"]
                if RDS_Data["DBClusterIdentifier"] != "":
